...
void verify(int p, int q, int g, int s1, int r1, int y, int message)
{
    int w, u1, u2, v, hash, term, choice;
    w = inverse(s1, q);
    printf("Enter the xor term: ");
    scanf("%d", &term);
    printf("Do you want the correct message or incorrect ? 0: correct 1: incorrect");
    scanf("%d", &choice);
    if(choice == 1){
    	printf("enter incorrect message: ");
    	scanf("%d", &message);
    }
    hash = message ^ term;
